...
import plugin.na.skusku.communication.OAuthConnect;

/**
 * The Class Login is the first page of the Wizard. It handles calls the
 * OAuthConnect class which connects the user to the server. Cant be finished or
 * flipped to next page if the user wasnt logged in succesfully.
 */
public class Login extends WizardPage {
	...
	public Login() {
		super("Page one");
		setTitle("Project Newcastle Build Process aaaaaaaaaaaaaaaaaaaaaaaaaa");
		setDescription("Here you can override default PNC server settings.");
	}

	/**
	 * Creates contents of the wizard. Contains btnLogin which contains
	 * updateButtons() method. This method calls the canFlipToNextPage() which
	 * logs user to server.
	 *
	 * @param parent
	 *            the parent
	...
	public void createControl(Composite parent) {
		...
		textURL.setText(
				"https://ncl-keycloak.stage.engineering.redhat.com/auth/realms/pncredhat/protocol/openid-connect/token");
	...
	/**
	 * If the login was successful the user can then click next button. The
	 * method won't try to login if the page is not completely filled with
	 * needed data.
	 */
	@Override
